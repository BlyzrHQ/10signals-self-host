...
export type DirectSearchScope = { allowedRivalDomains?: string[]; closePrice?: { basis: string; tolerancePercent: number; currency: string; primaryAmount: number; minimumAmount: number; maximumAmount: number } };
        ...
        input: [
          ...
          { role: "user", content: JSON.stringify({
            task: retainDistinctProductUrls
              ...
              : prompt.task,
            lane,
            ...(scope.closePrice ? { pricePositioning: { ...scope.closePrice,
              instruction: "Search for alternatives of the same product type in this displayed-price range and currency. Use price and market-position wording in queries while preserving material, contents, size and region. Do not relax the range to fill the target or substitute bulk/wholesale/unit prices. Price similarity is not proof of quality. Return product URLs; fetched seller evidence will independently verify prices." } } : {}),
...
export async function searchDirectProductPages(primaryDomainValue: string, primary: ProductRecord, marketCountryCode = "", repairFeedback?: ReportQualityRepairFeedback, scope: DirectSearchScope = {}, modelOverride?: string): Promise<DirectProductPageSearchResult> {
  ...
  const model = modelOverride || process.env.MARKET_SIGNAL_DISCOVERY_MODEL || "gpt-5.4-mini";
