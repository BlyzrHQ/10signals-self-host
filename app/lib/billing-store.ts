...
import { ensureReportCreditGrantSchema, reportCreditBonus } from "./report-credit-grants.mjs";
...
export function ensureBillingSchema(database: Database.Database): void {
  ...
  ensureReportCreditGrantSchema(database);
...
export function reserveReport(
  ...
  return database.transaction((): ReportReservation | null => {
    ...
    const limit = plan.reportsPerMonth + reportCreditBonus(database, workspaceId, subscription);
    if (existing) {
      ...
      return { id: existing.id, plan, used, limit, quotaKind: "reports" };
    }
    if (used >= limit) return { id: "", plan, used, limit, quotaKind: "reports", denialReason: "period-limit" };
    const id = randomUUID();
    ...
    return { id, plan, used: used + 1, limit, quotaKind: "reports" };
  }).immediate();
}
...
export function workspaceUsage(database: Database.Database, workspaceId: string, excludingCommandId = ""): { used: number; limit: number } {
  ...
  const ownReservation = excludingCommandId
    ...
    : { count: 0 };
  return { used: Math.max(0, Number(row.count || 0) - Number(ownReservation.count || 0)), limit: plan.reportsPerMonth + reportCreditBonus(database, workspaceId, subscription) };
}
