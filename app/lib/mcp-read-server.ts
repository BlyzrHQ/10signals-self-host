...
function safeToolFailure(error: unknown, operation: string): CallToolResult {
  ...
  console.error("10Signals MCP read tool failed.", {
    errorCode: "mcp-read-tool-failed",
    operation,
    errorName: error instanceof Error ? error.name : "unknown",
  });
  return failedToolResult("temporarily-unavailable", "10Signals data is temporarily unavailable.");
}
...
function registerAccountStatusTool(server: McpServer, principal: McpPrincipal, scopes: string[], services: McpWriteToolServices) {
  server.registerTool(
    "account_status",
    {
      title: "Get 10Signals account status",
...
function registerReportWriteTools(server: McpServer, principal: McpPrincipal, services: McpWriteToolServices) {
  server.registerTool(
    ...
    {
      title: "Preview a 10Signals report",
  ...
  server.registerTool(
    ...
    {
      title: "Confirm a 10Signals report",
...
function registerReportTools(server: McpServer, principal: McpPrincipal, services: McpReadServices) {
  server.registerTool(
    "reports_list",
    {
      title: "List 10Signals reports",
      description: "List a bounded page of private reports owned by the connected 10Signals account.",
      inputSchema: z.object({
...
export function createTenSignalsMcpHandler(
  ...
  return createMcpHandler(({ authInfo }: { authInfo?: AuthInfo }) => {
    ...
    legacy: "reject",
    responseMode: "auto",
    onerror: () => console.error("10Signals MCP protocol request failed.", { errorCode: "mcp-protocol-error" }),
  });
}
