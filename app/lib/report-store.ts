...
import { parseReportResearchOptions, compatibleCreationResearchOptions, type ReportResearchOptions } from "../../src/shared/web-direct-report-contract.ts";
...
const REPORT_RUN_COLUMN_MIGRATIONS = [
  ["research_options_json", "ALTER TABLE report_runs ADD COLUMN research_options_json text DEFAULT '' NOT NULL"],
...
function rowRun(row: Record<string, unknown>): StoredReportRun {
  ...
  return {
    ...
    ...(row.research_options_json ? { researchOptions: parseReportResearchOptions(JSON.parse(String(row.research_options_json))) } : {}),
  };
}
...
async function reportForMcpCommand(database: D1DatabaseLike, commandId: string, workspaceId: string) {
  ...
  const rows = await database.prepare(`SELECT
      ...
      runs.attempt_count, runs.created_at, runs.expires_at, runs.research_options_json,
...
async function createReportRunWithReplay(input: { primaryDomain: string; locale?: string; entitlement?: ProductEntitlement; workspaceId?: string; billingReservationId?: string; commandId?: string; researchOptions?: ReportResearchOptions }, now = new Date(), databaseOverride?: D1DatabaseLike | null) {
  const database = databaseOverride === undefined ? await getDatabase() : databaseOverride;
  if (!database) throw new Error(STORAGE_UNAVAILABLE_MESSAGE);
  ...
  const researchOptions = input.researchOptions ? parseReportResearchOptions(input.researchOptions) : undefined;
  await ensureSchema(database);
  const existing = await reportForMcpCommand(database, commandId, workspaceId);
  if (existing) {
    if (existing.primaryDomain !== primaryDomain || existing.locale !== locale || existing.productPlan !== productPlan || existing.productLimit !== productLimit || !compatibleCreationResearchOptions(existing.researchOptions, researchOptions)) {
  ...
  try {
    await database.batch([
      database.prepare(`INSERT INTO report_runs (id, public_id, primary_domain, locale, workspace_id, billing_reservation_id, mcp_command_id, status, current_phase, attempt_count, created_at, updated_at, heartbeat_at, expires_at, error_code, error_message, research_options_json) VALUES (?, ?, ?, ?, ?, ?, ?, 'queued', 'queued', 1, ?, ?, ?, ?, '', '', ?)`).bind(id, shareId, primaryDomain, locale, workspaceId, String(input.billingReservationId || ""), commandId, observedAt, observedAt, observedAt, expiresAt, researchOptions ? JSON.stringify(researchOptions) : ""),
    ...
    if (replay) {
      if (replay.primaryDomain !== primaryDomain || replay.locale !== locale || replay.productPlan !== productPlan || replay.productLimit !== productLimit || !compatibleCreationResearchOptions(replay.researchOptions, researchOptions)) {
        throw new ReportStorageError("command-intent-conflict");
      }
  ...
  return {
    report: { id, publicId: shareId, primaryDomain, locale, status: "queued" as const, currentPhase: "queued" as const, attemptCount: 1, createdAt: observedAt, expiresAt, productPlan, productLimit, productTargetKind: "pairs" as const, ...(researchOptions ? { researchOptions } : {}) },
    replayed: false as const,
    dispatchRecorded: false,
  };
}

export async function createReportRun(input: { primaryDomain: string; locale?: string; entitlement?: ProductEntitlement; workspaceId?: string; billingReservationId?: string; commandId?: string; researchOptions?: ReportResearchOptions }, now = new Date(), databaseOverride?: D1DatabaseLike | null) {
  return (await createReportRunWithReplay(input, now, databaseOverride)).report;
}

export async function createReportRunResult(input: { primaryDomain: string; locale?: string; entitlement?: ProductEntitlement; workspaceId?: string; billingReservationId?: string; commandId?: string; researchOptions?: ReportResearchOptions }, now = new Date(), databaseOverride?: D1DatabaseLike | null) {
  try {
