...
export type TenSignalsLoopNode = {
  id: string;
...
export type TenSignalsLoopEdge = {
  from: string;
  to: string;
  on: string;
  guard: string | null;
...
export const MARKET_SIGNAL_LOOP_GRAPH = {
  ...
  ] satisfies TenSignalsLoopEdge[],
} as const;

export function validateTenSignalsLoopGraph() {
  const issues: string[] = [];
