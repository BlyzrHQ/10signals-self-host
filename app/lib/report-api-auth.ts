...
export function reportApiAuthenticationRequiredResponse(message = "Sign in to access this report.") {
  return Response.json(
    { ok: false, error: message, errorCode: "authentication-required" },
    {
      ...
      headers: {
        "cache-control": "no-store",
        "www-authenticate": selfHostedEnabled() ? "Bearer" : `Bearer resource_metadata="${REPORT_API_RESOURCE_METADATA}"`,
      },
    },
  );
}
...
export async function reportApiAccountContext(
  ...
  // An explicitly supplied credential must stand on its own. Never let a
  // browser cookie turn a revoked/read-only key into a more privileged request.
  if (!request.headers.has("authorization")) {
    const browserAccount = await accountContext(request);
    if (browserAccount) return browserAccount;
  }
  // Independent multi-user installs must never inherit the legacy shared token.
  if (selfHostedEnabled(environment)) return null;
