...
import { parseReportResearchOptions, type ReportResearchOptions } from "../../src/shared/web-direct-report-contract.ts";
  ...
  try {
    ...
    return {
      ...
      report: {
        ...
        productTargetKind: "pairs",
        ...(report.researchOptions ? { researchOptions: parseReportResearchOptions(report.researchOptions) } : {}),
      },
    };
  } catch {
    return { kind: "boundary-failed", diagnosticCode: "create-access-failed" };
...
export async function createReportCommand(input: ReportCommandInput, services: ReportCommandDependencies = reportCommandDependencies()): Promise<ReportCommandResult> {
  ...
  try {
    ...
    const creation = await consumeReportCreation(services.create, {
      ...
      locale: input.locale,
      researchOptions,
      ...(selfHostedEnabled() && !reservation ? { entitlement: { plan: ({ 20: "starter", 50: "solo", 500: "growth", 1000: "agency" } as const)[input.comparisonTarget || 20], productLimit: input.comparisonTarget || 20 } } : {}),
      ...(input.actor ? {
    ...
    try {
      ...
      const message = diagnosticCode === "local-provider-unavailable"
        ? "Research is not configured. Ask your operator to set their provider key and start the local research worker. No research was launched."
        : diagnosticCode === "local-worker-unavailable"
          ? "The local research worker is not ready. Ask your operator to configure their own provider key in the worker and start the research profile. No research was launched."
          : "The background report job could not be started.";
      return { ok: false, status: 503, error: message, errorCode: "dispatch-failed", publicId, diagnosticCode, stage: "dispatch" };
    }
