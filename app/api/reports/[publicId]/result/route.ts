...
import { buildTenSignalsLoopResult } from "../../../../lib/report-loop-result.ts";
import { TenSignalsLoopContractError } from "../../../../../src/shared/market-signal-loop-contract.ts";
import {
  getReportEvaluation,
  getStoredReport,
...
export async function getReportLoopResult(
  ...
  try {
    ...
    return Response.json(await buildTenSignalsLoopResult({ requestId, report, matches, evaluation }), { headers: PRIVATE_REPORT_HEADERS });
  } catch (error) {
    ...
    const inconsistent = error instanceof ReportLoopFactsError || error instanceof TenSignalsLoopContractError;
