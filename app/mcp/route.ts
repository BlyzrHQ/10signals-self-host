...
const productionHandler = createTenSignalsMcpHandler();
...
function authenticationFailure(error?: McpAccessTokenError) {
  ...
  return Response.json({
    error: code,
    error_description: error?.message || "A valid 10Signals MCP bearer token is required.",
  }, { status, headers });
}
...
export async function postTenSignalsMcp(
  request: Request,
  ...
  try {
    ...
    console.error("10Signals MCP authorization failed.", { errorCode: "mcp-authorization-failed" });
  ...
  try {
    ...
    console.error("10Signals MCP request failed.", { errorCode: "mcp-request-failed" });
...
export function nonPostTenSignalsMcp(services: Pick<McpRouteDependencies, "enabled"> = mcpRouteDependencies()) {
  if (!services.enabled()) return new Response(null, { status: 404, headers: PRIVATE_MCP_HEADERS });
  return Response.json({ error: "method_not_allowed", error_description: "Use POST for 10Signals MCP requests." }, {
    status: 405,
