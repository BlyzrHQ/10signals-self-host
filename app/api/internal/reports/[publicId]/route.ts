    ...
    async post(request: Request, context: RouteContext) {
      ...
      try {
        ...
        if (body.action === "document") {
          ...
          await terminal.settle(persisted.run, persisted.run.status);
          if (report.run.researchOptions?.engine !== "direct-trigger" && saved.evaluation?.status === "deterministic" && await reportEvaluationPilotEnabled({ primaryDomain: report.run.primaryDomain, publicReportId: id })) {
          ...
          if (report.run.researchOptions?.engine !== "direct-trigger" && await reportSearchChallengeEnabled()) {
