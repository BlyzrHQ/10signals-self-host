  ...
  return <main className="ds-page acct-page cli-page-ds" lang="en" dir="ltr">
    <div className="ds-frame">
      ...
      <section className="cli-hero">
        ...
        <p>The internal 10Signals CLI calls the tools in the team’s existing Trigger project directly. Request team approval before connecting. Approved agents receive structured results without a hosted account sign-in. Installing the client does not grant project access; external customers use their own account API or MCP connection.</p>
      </section>
      <section className="cli-grid" aria-label="Internal CLI overview">
        <article className="ds-card cli-step"><span>01</span><div><h2 className="ds-h3">Install and configure</h2><p>After team approval, follow the installation guide below. Run configure to verify and securely save the team-issued Trigger environment key at a hidden prompt. Never put a key in a command or share it in chat.</p><pre className="acct-code"><code>marketsignal-trigger configure</code></pre></div></article>
        ...
        <article className="ds-card cli-handoff"><p className="ds-kicker">The complete handoff</p><p>The guide covers team approval, client installation and approved report requests. Provider credentials and deployment access stay with the team; configure does not install task code or provision providers. External customers must not use our company Trigger key.</p><Link className="ds-btn ds-btn-primary ds-btn-pill" href="/docs/trigger-cli">Open installation and first-report instructions →</Link></article>
      </section>
    </div>
  </main>;
}
