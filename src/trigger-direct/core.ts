import { z } from "zod";
import { DIRECT_SEARCH_MODEL, searchDirectWorkflowProductPages } from "./search-model.ts";
...
import { closePriceSearchContext, closePriceMatch } from "../../app/lib/close-price.ts";
...
export const requestSchema = z.object({
  ...
  closePricePercent: z.number().int().min(1).max(90).optional(),
}).strict();
...
export function capabilities(searchConfigured: boolean) {
  return { contractVersion: "1", service: "market-signal-direct", status: "ready", websiteRequired: false,
    ...
    searchModel: DIRECT_SEARCH_MODEL,
    ...
    closePrice: { supported: true, requestField: "closePricePercent", defaultPercent: 30, minimumPercent: 1, maximumPercent: 90, basis: "first-observed-displayed-offer", currencyConversion: false },
...
export async function runDirectReport(input: unknown, deps: DirectDependencies, recommendations = true) {
  ...
  let comparison = await buildDirectProductSearchComparison(request.domain, [catalog], {
    ...
    ...(request.closePricePercent !== undefined ? { closePricePercent: request.closePricePercent, enforceCompatibility: true, maxRivalDomains: request.rivals } : {}),
    search: deps.search || ((...args) => searchDirectWorkflowProductPages(...args, request.closePricePercent !== undefined
      ? { closePrice: closePriceSearchContext(args[1], request.closePricePercent) || undefined } : {})),
    enrich: request.closePricePercent !== undefined ? enrich : cappedEnrich,
  });
  ...
  const comparisons = comparison.rows.flatMap((row) => row.matches.flatMap((match) => match.product && hasValidObservedRivalPrice(row.primary) && hasValidObservedRivalPrice(match.product)
    ? [{ primaryProduct: row.primary, rivalProduct: match.product, assessment: match.assessment || { method: "direct-web-search", claimType: "Inferred", verdict: "search_result" }, recommendation: match.decision?.actionPlan || null,
      ...(request.closePricePercent !== undefined ? { priceMatch: closePriceMatch(row.primary, match.product, request.closePricePercent) } : {}) }]
    : []));
