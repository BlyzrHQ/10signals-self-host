...
/** Exact product representation consumed by agent output; keep forward and
 * checkpoint-recovery projections identical. This does not mutate facts. */
export function reportFactProduct(fact: JsonRecord) {
  return { ...record(fact.metadata), id: fact.productId, domain: fact.domain,
    name: fact.name, normalizedName: fact.normalizedName, sourceUrl: fact.sourceUrl,
    imageUrl: fact.imageUrl, observedAt: fact.observedAt, priceSignals: fact.prices };
}

/** Opt-in pre-publication projection. The quality gate must see the same
 * bounded, crawl-enriched product facts that the final agent response sees. */
export function projectComparisonFactProducts(comparison: ProductComparison, crawlResults: CrawlFactResult[], referenceTimeMs: number): ProductComparison {
  const facts = productFacts(crawlResults, comparison, new Date(referenceTimeMs).toISOString())
    .map(item => canonicalReportFact("products", item) as JsonRecord);
  const byKey = new Map(facts.map(fact => [`${fact.domain}\n${fact.productId}`, fact]));
  const project = (product: ProductRecord) => {
    const fact = byKey.get(`${canonicalDomain(product.domain)}\n${product.id}`);
    if (!fact) throw new Error("PUBLICATION_PRODUCT_PROJECTION_MISSING");
    // Retain graph-only identity fields. All fields exposed by the response
    // are overwritten by the exact canonical fact projection.
    return { ...product, ...reportFactProduct(fact) } as ProductRecord;
  };
  return { ...comparison, rows: comparison.rows.map(row => ({ ...row,
    primary: project(row.primary), matches: row.matches.map(match => ({ ...match,
      ...(match.product ? { product: project(match.product) } : {}),
      ...(match.excludedProduct ? { excludedProduct: project(match.excludedProduct) } : {}),
    })),
  })) };
}
