export const MARKET_SIGNAL_ORIGIN = "https://10signals.xyz";
...
export const MCP_SCOPE_DETAILS: Record<McpResourceScope, { title: string; description: string }> = {
  "reports:read": {
    ...
    description: "View reports and comparison results owned by your 10Signals workspace.",
  },
  "reports:create": {
...
export function mcpClientIdentity(clientId: string, selfAssertedName?: string | null) {
  ...
  return {
    clientId,
    host,
    name: firstPartyCli ? "10Signals CLI" : String(selfAssertedName || "").trim() || host,
