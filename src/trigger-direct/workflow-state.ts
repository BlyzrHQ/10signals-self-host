...
export type WorkflowIdentity = { publicId: string; primaryDomain: string; locale: "en" | "ar"; attemptCount: number;
  productPlan: "starter" | "solo" | "growth" | "agency"; productLimit: number; createdAt: string };
function identityFields(run: StoredReport["run"]) {
  return { publicId: run.publicId, primaryDomain: run.primaryDomain, locale: run.locale, attemptCount: run.attemptCount,
    productPlan: run.productPlan, productLimit: run.productLimit, createdAt: run.createdAt };
}
...
export function decodeState(packet: StatePacket, ownerRunId: string, request: DirectRequest, identity?: WorkflowIdentity): WorkflowState {
  ...
  if (state.version !== 1 || state.ownerRunId !== ownerRunId || state.revision !== packet.revision || hash(state) !== packet.hash
    ...
    || !state.operations || typeof state.operations !== "object"
    || (identity && hash(identityFields(state.report.run)) !== hash(identity))) throw new Error("STATE_INTEGRITY_CONFLICT");
  return state;
}
export function initialState(ownerRunId: string, request: DirectRequest, observedAt = new Date().toISOString(), identity?: WorkflowIdentity): WorkflowState {
  if (identity && (!/^[a-f0-9]{32}$/.test(identity.publicId) || identity.primaryDomain !== request.domain || identity.productLimit !== request.comparisons
    || !Number.isInteger(identity.attemptCount) || identity.attemptCount < 1 || identity.attemptCount > 20 || !Number.isFinite(Date.parse(identity.createdAt)))) throw new Error("INVALID_WORKFLOW_IDENTITY");
  const publicId = hash({ ownerRunId, request }).slice(0, 32);
  return { version: 1, ownerRunId, request, revision: 0, report: {
    run: { publicId, primaryDomain: request.domain, locale: "en", status: "queued", attemptCount: 1, productPlan: "starter", productLimit: request.comparisons, createdAt: observedAt, updatedAt: observedAt, ...(identity || {}) }, events: [], factManifest: null,
...
export class WorkflowStore {
  ...
  identity(publicId: string, attemptNumber = this.state.report.run.attemptCount) {
    this.assertHealthy();
    if (publicId !== this.state.report.run.publicId || attemptNumber !== this.state.report.run.attemptCount) throw new Error("WORKFLOW_IDENTITY_CONFLICT");
  }
  ...
  async saveCheckpoint(publicId: string, input: ReportMatchBatchCheckpointInput | ReportMatchBatchCheckpointReplaceInput) {
    ...
    // The engine binds terminal presentations to JSON.stringify(result).
    // Preserve field order; canonical hashing below still protects state/replays.
    const result = plain(input.result);
    ...
    return this.update((state) => {
      ...
      const checkpoint = { attemptNumber: state.report.run.attemptCount, batchIndex: input.batchIndex, inputHash: input.inputHash, result, resultHash, createdAt: existing?.createdAt || timestamp, updatedAt: timestamp };
  ...
  finalizeFactManifest: ReportOrchestrationPort["finalizeFactManifest"] = async (publicId, input) => {
    ...
    await this.update(async (state) => {
      ...
      state.report.factManifest = { ...input, attemptNumber: state.report.run.attemptCount, status: "complete", completedAt: new Date().toISOString() };
    });
  };
