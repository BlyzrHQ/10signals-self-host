...
import { reportApiAccountContext, ReportApiAuthorizationError, reportApiAuthorizationErrorResponse } from "../../../lib/report-api-auth.ts";
...
export function reportRouteDependencies(): ReportRouteDependencies {
  return {
    ...
    recover: (...args) => selfHostedEnabled() ? Promise.resolve(null) : recoverLegacyReport(...args),
    authorize: reportApiAccountContext,
    settle: settleTerminalReportReservation,
    allowLegacyPublic: () => !hostedBillingEnabled(process.env) && !selfHostedEnabled(),
  };
}
