...
import { commitStatePointer, decodeState, encodeState, initialState, inlineStatePointer, WorkflowStore, type StatePacket, type StatePointer, type WorkflowIdentity } from "./workflow-state.ts";
...
export async function openWorkflowStore(ownerRunId: string, request: DirectRequest, workerVersion: string, attemptNumber: number, identity?: WorkflowIdentity) {
  ...
  if (pointer) {
    ...
    state = decodeState(packet, ownerRunId, request, identity);
    ...
    state = initialState(ownerRunId, request, identity?.createdAt, identity);
  }
...
export async function runWorkflow(payload: unknown, context: { runId: string; workerVersion: string; attemptNumber: number; maxAttempts: number }) {
  ...
  const store = await openWorkflowStore(context.runId, request, context.workerVersion, context.attemptNumber);
