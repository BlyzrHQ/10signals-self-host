...
import type { ReportResearchOptions } from "../../../../src/shared/web-direct-report-contract.ts";
...
type AcceptanceReportServices = {
  ...
  create(input: { primaryDomain: string; locale: "en" | "ar"; entitlement: { plan: ProductPlan; productLimit: number }; researchOptions: ReportResearchOptions }): ReturnType<typeof createReportRunResult>;
...
export function createAcceptanceReportHandler(services: AcceptanceReportServices = liveServices) {
  return async function post(request: Request) {
    ...
    const creation = await services.create({
      ...
      // Acceptance must exercise the same default engine as new customer reports.
      researchOptions: { engine: "direct-trigger", includeAnalysis: false },
    });
    if (!creation.ok) {
