...
export function CompetitorPriceWatch({ publicId, matchesEndpoint, rivals, onlyDomain, ar }: CompetitorPriceWatchProps) {
  ...
  const rivalRows = useMemo(() => {
    ...
    return [...merged.entries()].filter(([domain]) => !onlyDomain || domain === onlyDomain).map(([domain, count]) => ({ domain, count }));
  }, [domainCounts, hasAuthoritativeCounts, rivals, onlyDomain]);
