...
export type TenSignalsLoopPending = {
  state: "pending";
  requestId: string;
...
export type TenSignalsLoopTerminal = {
  state: "terminal";
  output: TenSignalsLoopOutput;
  decision: {
    headline: string;
...
export type TenSignalsLoopResult = TenSignalsLoopPending | TenSignalsLoopTerminal;
...
export async function buildTenSignalsLoopResult(input: {
  ...
  const outputStatus: TenSignalsLoopOutput["status"] = run.status === "interrupted"
    ? "outcome_unknown"
    : run.status === "failed"
      ? "failed"
