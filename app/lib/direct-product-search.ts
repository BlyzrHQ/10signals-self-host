...
import { closePriceMatch, closePriceSearchContext, validateClosePricePercent } from "./close-price.ts";
import { representativeProductOrder } from "./product-research-order.ts";
import { enrichmentGapMessage } from "./product-price-coverage.ts";
...
export type DirectProductSearchOptions = {
  ...
  /** Optional displayed-price positioning; omitted preserves broad searches. */
...
function searchInputHash(primaryDomain: string, primary: ProductRecord, marketCountryCode: string, repairFeedback?: ReportQualityRepairFeedback, closePricePercent?: number) {
  return createHash("sha256").update(JSON.stringify({
    ...
    ...(closePricePercent !== undefined ? { closePrice: { version: 1, percent: closePricePercent, prices: primary.priceSignals, quantity: primary.quantity, attributes: primary.attributes } } : {}),
  })).digest("hex");
}
...
export async function buildDirectProductSearchComparison(primaryDomainValue: string, catalogs: Array<{ domain: string; products: ProductRecord[] }>, options: DirectProductSearchOptions): Promise<ProductComparison> {
  if (options.closePricePercent !== undefined) validateClosePricePercent(options.closePricePercent);
  ...
  const search: DirectSearch = options.search || ((domain, primary, country, feedback) => searchDirectProductPages(domain, primary, country, feedback,
    options.closePricePercent !== undefined ? { closePrice: closePriceSearchContext(primary, options.closePricePercent, referenceTimeMs) || undefined } : {}));
  ...
  const searchProducts = representativeProductOrder(primaryProducts
    ...
    .filter(({ primary }) => (!repairPrimaryIds || repairPrimaryIds.has(primary.id)) && searchablePrimaryProduct(primary, referenceTimeMs)
      && (options.closePricePercent === undefined || closePriceSearchContext(primary, options.closePricePercent, referenceTimeMs) !== null)));
  const searchPriority = new Map(searchProducts.map(({ primary }, index) => [primary.id, index]));
  ...
  const assignOutcomes = () => {
    ...
    const compatible = (primary: ProductRecord, rival: ProductRecord) => {
      ...
      if (options.closePricePercent !== undefined) {
        const result = closePriceMatch(primary, rival, options.closePricePercent, referenceTimeMs);
        if (!result.eligible) {
          const gap = `${primary.name}: excluded ${rival.sourceUrl} (${result.reason}; close-price +/-${options.closePricePercent}%).`;
          if (!gaps.includes(gap)) gaps.push(gap);
          return false;
        }
      }
  ...
  const processPrimary = async ({ primary, primaryIndex }: { primary: ProductRecord; primaryIndex: number }) => {
    ...
    const key = { primaryIndex, inputHash: searchInputHash(primaryDomain, primary, marketCountryCode, options.repairFeedback, options.closePricePercent) };
    ...
    const completedCheckpoint = validSearchCheckpoint({
      ...
      outcome: {
        ...
        gaps: enriched.coverage.gaps.slice(0, 12).map(enrichmentGapMessage),
      },
    }, primary, referenceTimeMs);
  ...
  for (let offset = 0; offset < searchProducts.length; offset += concurrency) {
    ...
    outcomes.sort((a, b) => (searchPriority.get(a.primary.id) ?? 0) - (searchPriority.get(b.primary.id) ?? 0));
    assignOutcomes();
  }
