...
import { searchDirectWorkflowProductPages } from "./search-model.ts";
...
import { closePriceSearchContext } from "../../app/lib/close-price.ts";
import { constrainClosePrice } from "./close-price-policy.ts";
import { projectComparisonFactProducts } from "../shared/report-facts.ts";
import { publicationComparisons } from "./workflow-output.ts";
...
export function createWorkflowPort(store: WorkflowStore, research: {
  ...
  const handler = createMatchHandler({
    loadEntitlement: async (id, attempt) => { store.identity(id, attempt); const run = store.read().report.run; return { plan: run.productPlan || "starter", productLimit: run.productLimit!, reportObservedAt: run.createdAt }; },
    ...
    buildDirect: (domain, catalogs, options) => buildDirectProductSearchComparison(domain, catalogs, { ...options,
      ...
      closePricePercent: store.read().request.closePricePercent,
      ...
      search: (...args) => {
        ...
        const percent = store.read().request.closePricePercent;
        const scope = { ...(args[3] && retainedRivalDomains.length >= store.read().request.rivals
          ? { allowedRivalDomains: [...retainedRivalDomains].sort() } : {}),
          ...(percent !== undefined ? { closePrice: closePriceSearchContext(args[1], percent) || undefined } : {}) };
        return store.operation(`search:${hash(scope.allowedRivalDomains || percent !== undefined ? { args, scope } : args)}`, async () => {
        const result = await (research.search || searchDirectWorkflowProductPages)(...args, scope);
  ...
  return {
    ...
    constrainPublishedComparison: (comparison, crawlResults = [], referenceTimeMs = Date.now()) => {
      if (store.read().request.closePricePercent !== undefined) comparison = projectComparisonFactProducts(comparison, crawlResults, referenceTimeMs);
      comparison = constrainClosePrice(comparison, store.read().request.closePricePercent, store.read().request.comparisons, referenceTimeMs);
    ...
    validatePublicationFacts: (facts, observedAt) => { publicationComparisons(store.read().request, facts.chunks, facts.manifest.manifestId, observedAt); },
